...
gsl_vector_complex* diffusion_gate(gsl_vector_complex* wavefunction){
    ...
    gsl_matrix_complex_set(diffusion_gate, 0, 0, gsl_complex_rect(-1,0)); //Minus one as index from 0
    
    gsl_vector_complex* o_psi = gsl_vector_complex_alloc(wavefunction->size);
    gsl_vector_complex_set_zero(o_psi);
    gsl_blas_zgemv(CblasNoTrans, GSL_COMPLEX_ONE, diffusion_gate, wavefunction, GSL_COMPLEX_ZERO, o_psi);
