...
gsl_vector_complex* findElementFofx(int control, int a, int C, gsl_vector_complex* wavefunction){ // only need to cycle through rows as permutation matrix
    ...
    gsl_spmatrix_int* amodx = gsl_spmatrix_int_alloc(wavefunction->size, wavefunction->size);
    ...
    //sum through rows and cols to show their unity
    for(int d = 0; d < amodx->size1; d++){
        int val = 0;
        for(int r = 0; r< amodx->size2; r++){
            val += gsl_spmatrix_int_get(amodx, d, r);
            printf("%d", val);
        }
    }
