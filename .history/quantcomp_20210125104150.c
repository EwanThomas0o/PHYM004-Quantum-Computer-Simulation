...
void hadamard_gate(gsl_vector_complex* wavefunction, int qubit){
    ...
    if (qubit == 3){
        ...
        gsl_matrix_set(hadamard, 5,5, -1);
        gsl_matrix_set(hadamard, 7,7, -1);
        gsl_matrix_set(hadamard, 1,0, 1);
        gsl_matrix_set(hadamard, 0,1, 1);
        gsl_matrix_set(hadamard, 2,3, 1);
        gsl_matrix_set(hadamard, 3,2, 1);
        gsl_matrix_set(hadamard, 4,5, 1);
        gsl_matrix_set(hadamard, 5,4, 1);
        gsl_matrix_set(hadamard, 6,7, 1);
        gsl_matrix_set(hadamard, 7,6, 1);
        gsl_matrix_scale(hadamard, 1/sqrt(BASIS));
    }
    gsl_vector_complex *new_wavefunction = wavefunction;

    gsl_blas_dgemv(CblasNoTrans, 1.0, hadamard, wavefunction, 0.0, wavefunction);
    return;
}
