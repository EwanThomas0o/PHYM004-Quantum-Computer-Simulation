...
void hadamard_gate(gsl_vector_complex* wavefunction, int qubit){
    const gsl_matrix_complex *hadamard = gsl_matrix_alloc(wavefunction->size, wavefunction->size);
    ...
    gsl_blas_cgemv(CblasNoTrans, z, hadamard, wavefunction, z, wavefunction);
    return;
}
