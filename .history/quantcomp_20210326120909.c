...
// Purpose
// -------
// This files stores all function used to simulate a quantum register for 
// quantum computation. 
// 
// Usage
// -----
// Compile with command "make" and execute with "make do" to execute binary for test.c as this
// is where sandbox mode and other demo modes are located.
//
/*      UPDATES
        -------
...
// malloc wrapper to check if mem alloc failed 
// 
// Arguments
// ---------
// [1] bits -> amount of memory required
...
// Creates quantum register with defined amount of qubits
//
// Argumnents
// ----------
// [1] qubits -> number of qubits 
//
// Returns
// -------
// [1] quantumRegister -> the register used to define the system
gsl_vector_complex* createRegister(int qubits){
    ...
    if(qubits > 12){
        fprintf(stderr, "Warning: Computations with more than 12 qubits are memory and time intensive");
    }
    gsl_vector_complex* quantumRegister = gsl_vector_complex_alloc(pow(2, qubits));
    gsl_vector_complex_set_basis(quantumRegister, 0);

    printf("%d qubits initialised all to spin down", qubits);
...
gsl_vector_complex* spCompMatCompVec(const CBLAS_TRANSPOSE_t TransA,
  ...
  if ((TransA == CblasNoTrans && K != x->size) ||
  ...
  else
    {
      ...
      if (GSL_SPMATRIX_ISTRIPLET(A))
        ...
        {
          printf("Error: spmatrix type must be COO");  
          return NULL;
...
gsl_vector_complex* initWavefunctionEqualProb(int states){ //Initialising wf to "Equal Probability" of all states
    ...
    gsl_vector_complex_set_all(wavefunction , gsl_complex_rect(1/sqrt(states), 0));
...
char* intToBinary(int a, gsl_vector_complex* wavefunction){
    ...
    while(a != 0){
        ...
        remainder = a % 2;
        a /= 2;
        bin[qubits-index-1] = remainder*temp; //Gives qubit-places.
        index++;
    }
    char *bin_str = (char *) calloc(qubits, sizeof(char));
...
char* measureRegisterGate(gsl_vector_complex* wavefunction){
    ...
    return intToBinary(t, wavefunction);
...
double findElementHad(char* inta, char* intb, int qubit, gsl_vector_complex* wavefunction){
...
gsl_complex findElementPhase(char* inta, char* intb, int qubit, double phi, gsl_vector_complex* wavefunction){
...
void amodcGate(int control, int a, int C, gsl_vector_complex* wavefunction){ // only need to cycle through rows as permutation matrix
    ...
    for(int k = 0; k < amodx->size2; k++){
        ...
        char* binK = intToBinary(k, wavefunction);
        ...
        if (binK[control-1] - '0' != 0)
        {
            ...
            else if(binK[control-1] - '0' != 0 && (int)strtol(f, NULL, 2) < C)
            {
                ...
                char* binFprime = xmalloc(qubits); // a char is one byte

                binFprime = intToBinary(fprime, wavefunction);

                char *l = xmalloc(qubits);
...
void CphaseGate(gsl_vector_complex* wavefunction, int control, int target, double phase){
    ...
    /* Will need to only put val if val is non-zero due to sparse nature, need to use spCompMatCompVec in order to multiply
    complex matrix by complex vector */
    int qubits = log2(wavefunction->size);

    if(control == target){
        printf("Error: control and target cannot be on the same qubit");
        exit(0);
    }
    ...
    for(int i = 0; i < wavefunction->size; i++){
        for(int j = 0; j < wavefunction->size; j++){
            gsl_complex val = findElementCphase(intToBinary(i, wavefunction), intToBinary(j, wavefunction), control, target, qubits, phase);
    ...
    spCompMatCompVec(CblasNoTrans, cphasegate, wavefunction, r_psi); // complex sparse multiplier, works with COO only
...
void hadamardGate(gsl_vector_complex* wavefunction, int qubit){
    ...
    for(int i = 0; i < wavefunction->size; i++){
        ...
        for(int j = 0; j < wavefunction->size; j++){
            ...
            double val = findElementHad(intToBinary(i, wavefunction), intToBinary(j, wavefunction), qubit, wavefunction);
...
void phaseShiftGate(gsl_vector_complex *wavefunction, int qubit, double phase){
    ...
    for(int i = 0; i < wavefunction->size; i++){
        for(int j = 0; j < wavefunction->size; j++){
            gsl_complex val = findElementPhase(intToBinary(i, wavefunction), intToBinary(j, wavefunction), qubit, phase, wavefunction);
            if(GSL_REAL(val) == 0 && GSL_IMAG(val) == 0){
                /* Do Nothing */
            }
            else{
    ...
    spCompMatCompVec(CblasNoTrans, phaseGate, wavefunction, r_psi); // complex sparse multiplier, works with COO only
...
// Applys all gates nessecary to run Grover's Algorithm
//
// Arguments
// ---------
// [1] quantum_register -> Defines the state of the system with probability amplitudes of possible configurations
// [2] Answer -> The correct state in grovers alg
void groversAlgorithm(gsl_vector_complex* quantum_register, int answer){
    int qubits = log2(quantum_register->size);
    
    if(answer > quantum_register->size)
    {
        fprintf(stderr, "Error: answer state \"%d\" cannot be represented with %d qubits", answer, qubits);
        exit(0);
    }
    ...
    for(int j = 0; j < floor(M_PI_4*sqrt(pow(2,qubits))); j++){ // Needs to be called "floor(pi/4*sqrt(2^N))"" times for optimum output roughly 2 in our case
        groversBlock(quantum_register, answer); //Second argument is the basis state you want to be "right" in this case its |110>
    }
}
...
void groversBlock(gsl_vector_complex* wavefunction, int answer){
    int qubits = log2(wavefunction->size);
    gsl_vector_complex* b_psi = gsl_vector_complex_calloc(wavefunction->size);
    b_psi->data = wavefunction->data;

    //First operation in the block is to apply a quantum oracle gate
    oracleGate(b_psi, answer); 
    //Then we apply a hadamard gate to each gate
    for(int i = 1; i <= qubits; i++){
        hadamardGate(b_psi, i);      
    }
    // Apply the j gate
    jGate(b_psi);
    // Finally a hadamard gate on each qubit again
    for(int j = 1; j <= qubits; j++){
...
void cnotGate(gsl_vector_complex* wavefunction, int control, int target){
    ...
    for(int i = 0; i < wavefunction->size; i++){
        for(int j = 0; j < wavefunction->size; j++){
            gsl_spmatrix_set(cnot, i, j, findElementCnot(intToBinary(i, wavefunction), intToBinary(j, wavefunction), control, target, qubits));
        }
    }
...
void shorsBlock(gsl_vector_complex* wavefunction, int rand, int compositeNumber){
    ...
    gsl_vector_complex* s_psi = gsl_vector_complex_calloc(wavefunction->size);
    s_psi = initWavefunctionShors(wavefunction->size);

    for(int xQubitHad = 1; xQubitHad <= L; xQubitHad++){
     
        hadamardGate(s_psi, xQubitHad);
    
    }
    // Multiplying f register by f(x)
    for(int xQubitCGate = L; xQubitCGate > 0; xQubitCGate--){
    
        amodcGate(xQubitCGate, rand, compositeNumber, s_psi);
    
    }
    // IQFT block--------------------------------------------
    hadamardGate(s_psi, 1); 
    CphaseGate(s_psi, 1, 2, M_PI_2);
    CphaseGate(s_psi, 1, 3, M_PI_4);
    hadamardGate(s_psi, 2);
    CphaseGate(s_psi, 2, 3, M_PI_2);
    hadamardGate(s_psi, 3);

    swapsies(s_psi, wavefunction);
...
primeFactors shors(gsl_vector_complex* wavefunction, int compositeNumber){
    int qubits = log2(wavefunction->size);
    if(qubits != 7){
        printf("Error: this implementation of Shor's Algorithm only works for 7 qubits");
        exit(0);
    }
