...
double findElementCnot(char* row, char* col, int target_qubit, int control_qubit, int num_qbits){
    ...
    gsl_matrix_complex *cnot = gsl_matrix_complex_alloc(BASIS, BASIS);
    gsl_matrix_complex_set(cnot, 0, 0, GSL_COMPLEX_ONE);
    gsl_matrix_complex_set(cnot, 1, 1, GSL_COMPLEX_ONE);
    gsl_matrix_complex_set(cnot, 2, 3, GSL_COMPLEX_ONE);
    gsl_matrix_complex_set(cnot, 3, 2, GSL_COMPLEX_ONE);

    for(int i = 0; i < num_qbits; i++){
        // Employ the deltas first
        if(i!= target_qubit-1 && i!= control_qubit-1 && strcmp(&row[i], &col[i])!= 0 ){
    ...
    // use strtol to make a number that will give us a value from cnot matrix above
    long row_index = strtol(str1, NULL, 10);
    long col_index = strtol(str2, NULL, 10);
    double value = GSL_REAL(gsl_matrix_complex_get(cnot, row_index, col_index));
    printf("%lg", value);
    return value;
...
int main(){
    ...
    findElementCnot(intToBinary(7),intToBinary(4), 3, 1, N);
