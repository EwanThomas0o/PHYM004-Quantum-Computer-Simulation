...
int findElementFofx(char* col, int control, int a, int C, gsl_vector_complex* wavefunction){ // only need to cycle thru rows as permutation matrix
    double A = (int)pow((double)a, (double)control) % C;
    
    gsl_spmatrix* amodx = gsl_spmatrix_alloc(wavefunction->size, wavefunction->size);
    
    char* binK = intToBinary(col); // writing k as (l2,l1,l0,m3,m2,m1,m0)_2
