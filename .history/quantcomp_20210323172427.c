...
#define BASIS 2
#define N 6 // Number of qubits defined
...
void phaseShiftGate(gsl_vector_complex *wavefunction, int qubit, double phase){
    ...
    for(int i = 0; i < wavefunction->size; i++){
        for(int j = 0; j < wavefunction->size; j++){
            gsl_complex val = findElementPhase(intToBinary(i, wavefunction), intToBinary(j, wavefunction), qubit, phase);
            if(GSL_REAL(val) == 0 && GSL_IMAG(val) == 0){
                /* Do Nothing */
            }
            else{
...
void cnotGate(gsl_vector_complex* wavefunction, int control, int target){
    ...
    for(int i = 0; i < wavefunction->size; i++){
        for(int j = 0; j < wavefunction->size; j++){
            gsl_spmatrix_set(cnot, i, j, findElementCnot(intToBinary(i, wavefunction), intToBinary(j, wavefunction), control, target, N));
        }
    }
