...
double findElementCnot(char* row, char* col, int target_qubit, int control_qubit, int num_qbits){
    ...
    printf("%s\n", row);
    printf("%s\n", col);
    str1[0] = (char)row[control_qubit-1];
    // printf("%s\n", str1);
    str1[1] = (char)row[target_qubit-1] - '0';
    // printf("%s\n", str1);
    str2[0] = (char)col[control_qubit-1] - '0';
    str2[1] = (char)col[target_qubit-1] - '0';

    printf("row in binary = %s, col in binary = %s\n", str1, str2);
