...
int testP(int a, int p, int C){
    ...
    double congruence = ( (int)pow((double)a, (double)p/2) + 1 ) % C;
    double oneCongruence;
    // printf("%lg", congruence);
    
    if(p % 2 != 0 && congruence != floor(congruence) ){
        return 1;
    }
    else
    {
...
primeFactors shors(gsl_vector_complex* wavefunction, int composite_number){
    ...
    if(isp != 1){
        factors.a = isp;
        factors.b = (int) composite_number / isp;
    ...
    int gcf = greatestCommonDivisor(rand, composite_number);
    
    if( gcf > 1){
        factors.a = gcf;
        factors.b = (int)composite_number/gcf;
        printf("factor a = %d \nfactor b = %d\n", factors.a, factors.b);
    ...
    while(omega == 0){
        ...
        for(int xQubitCGate = L; xQubitCGate > 0; xQubitCGate--){
            ...
            wavefunction = amodcGate(xQubitCGate, rand, composite_number, wavefunction); // we use the same number until we don't measure a 0
    ...
    {
        double k = (double)((int)pow((double)rand, (double)i*p) % composite_number); //Won't this always give me an int and hence ciel(k) always = k??
        double oneCongruence = 1 % composite_number;

        printf("Check to seee if 1 mod(15) = a^p mod(15)\nk  = %lg and 1mod(15) = %lg", k, oneCongruence);
        
        if(ceil(k) == k)
        {
            int test = testP(rand, i*p, composite_number);
            printf("test =%d\n", test);
            if(test == 1){
                factors.a = greatestCommonDivisor( pow(rand, (i*p)/2) + 1, composite_number);
                factors.b = greatestCommonDivisor( pow(rand, (i*p)/2) - 1, composite_number);
                printf("a =%d b=%d\n", factors.a, factors.b);
