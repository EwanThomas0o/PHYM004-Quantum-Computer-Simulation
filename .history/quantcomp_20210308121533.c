...
int findElementFofx(int control, int a, int C, gsl_vector_complex* wavefunction){ // only need to cycle through rows as permutation matrix
    ...
    for(int k = 0; k < amodx->size2; k++){
        ...
        {
            char* f = malloc(4);
            strncpy(f, binK+3, 4); //taking the substring m3m2m1m0
            ...
            {
               int fprime = A*(int)strtol(f, NULL, 10); 
            }
