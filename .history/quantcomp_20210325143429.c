...
#define BASIS 2
#define STATES_MAX 10 //max of 10 qubits 
#define L 3
#define M 4
...
void groversAlgorithm(gsl_vector_complex* quantum_register, int answer){
    ...
    for(int j = 0; j < floor(M_PI_4*sqrt(pow(2,qubits))) + 1; j++){ // Needs to be called "floor(pi/4*sqrt(2^N))"" times for optimum output roughly 2 in our case
