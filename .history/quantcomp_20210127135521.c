...
double findElement(long inta, long intb, int qubit){
    ...
    for(int i = 0; i < N; i++){
        if(str1[i] == str2[i]){
            match[i] = 1; //If they all match then the element wont be 0 due to deltas
        }
    //  Element match[qubit] is the hamiltonian term. Therfore is there if match[i] = 0 anywhere else in the array other than that match[qubit]
