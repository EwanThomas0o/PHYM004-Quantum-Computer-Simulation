...
gsl_vector_complex* createRegister(int qubits){
    if(qubits < 1)
    {
        fprintf(stderr, ANSI_COLOR_RED "Error: Qubit number must be greater than 0\n" ANSI_COLOR_RESET);
        exit(0);
    }
    if(qubits > 14){
        fprintf(stderr, ANSI_COLOR_PURPLE "Warning: Computations with more than 14 qubits are memory and time intensive\n" ANSI_COLOR_RESET);
    }
