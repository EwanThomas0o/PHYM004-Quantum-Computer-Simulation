...
gsl_vector_complex* createRegister(int qubits){
    if(qubits < 1)
    {
        fprintf(stderr, "Error: Qubit number must be greater 0");
        exit(0);
    }
    if(qubits > 14){
        fprintf(stderr, "Warning: Computations with more than 14 qubits are memory and time intensive");
    }
    gsl_vector_complex* quantumRegister = gsl_vector_complex_calloc(pow(2, qubits));
...
gsl_vector_complex* initWavefunctionSpinDown(gsl_vector_complex* wavefunction, int states){ 
