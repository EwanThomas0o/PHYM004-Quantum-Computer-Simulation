...
double findElement(long inta, long intb, int qubit){
    // Convert into str to analyse each digit
    char str1[N];
    char str2[N];
    sprintf(str1, "%ld", inta);
    sprintf(str2, "%ld", intb);
    ...
    int sum = 0;
    for(int i = 0; i < N; i++){
        if(str1[i] != str2[i] && i != qubit - 1){
            return value; //If they all match (except the H term)then the element wont be 0 due to deltas
        }
        else{
            int row = str1[qubit - 1];
            int col = str2[qubit - 1];

            value = GSL_REAL(gsl_matrix_complex_get(hadamard_single, row, col));
            return value
        }
    //  Element match[qubit-1] is the hamiltonian term. Therfore is there if match[i] = 0 anywhere else in the array other than that match[qubit - 1]
    // then the term wll be zero

    // Can loop through again as matricies won't exceed length of 7 in size
    for(int j = 0; j < N; j++){
        if(match[j] == 0 && j != qubit - 1){
            return 0.0;
        }
        else{
            element elem = 
    ...
    if (qubit == 1){
        for(int i = 0; i < wavefunction->size; i++){
            for(int j = 0; j < wavefunction->size; j++){
                element val = findElement(num_to_bin(i), num_to_bin(j), qubit);
