...
int readsXReg(gsl_vector_complex* wavefunction){
    //Wavefunction needs to be collapsed in order for this to work properly! Add a measure gate just to be sure
    //maybe add a measured flag to the wavefunction struct??
    // After wf is collapsed we measure which state it's in, turn this number into binary, then 
}
